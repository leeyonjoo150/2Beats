...
from apps.twobeats_account.models import MusicPlaylist, PlaylistTrack
...
def save_result_to_playlist(request, game_id):
    ...
    try:
        ...
        new_playlist = MusicPlaylist.objects.create(
            user=request.user,
        ...
        for index, result in enumerate(top_results):
            playlist_musics.append(PlaylistTrack(
                playlist=new_playlist,
                music=result.wc_music,
                order=index # 순서대로 저장
            ))
        
        PlaylistTrack.objects.bulk_create(playlist_musics)
